...
Od tamtego momentu React cały czas prężnie się rozwija, pozostaje jednym z~ najpopularniejszych narzędzi do tworzenia interfejsów graficznych, używany jest nie tylko przez ogromne firmy, takie jak Uber, Netflix, Airbnb, Amazon, Twitter, ale również przez zwykłych użytkowników. Popularność Reacta nie przestaje rosnąć, w styczniu 2021 roku biblioteka została pobrana ponad 10 milionów razy - zdecydowanie wyróżniając się na tle podobnych narzędzi przeznaczonych do tworzenia interfejsów użytkownika.
%###############################################################
\begin{figure}
\centering
...
\newline
Źródło: \url{https://npmtrends.com/angular-vs-react-vs-vue/}
}
...
Właśnie z tych powodów trybu developerskiego używamy w czasie budowania aplikacji, a trybu produkcyjnego w~ostatecznej wersji produktu, która ma trafić do użytkownika.
\begin{figure}[H]
\centering
...
\end{lstlisting}
W przypadku, gdy nad projektem pracuje wiele osób, ilość używanych bibliotek może rozrastać się dużo szybciej wraz z szukaniem nowych rozwiązań do pojawiających się problemów.
...
\end{lstlisting}
Jedną z zasad obowiązujących podczas budowy komponentów, zarówno funkcyjnych jak i klasowych, pozostaje nienaruszalność właściwości (ang. \textit{props}) przekazywanych podczas deklaracji funkcji. Dane, które mają zmieniać się dynamiczne powinny być przechowywane w stanach.
...
\end{figure}
Każdy cykl wywołuje inny zestaw metod, najważniejszymi metodami wywoływanymi w czasie montowania są \texttt{constructor} - odpowiedzialny za inicjalizację stanu lokalnego oraz związanie(ang.\textit{bind}) metody obsługi zdarzeń, render oraz \texttt{componentDidMount} - ta metoda jest odpowiednim miejscem do wysłania zapytania, jeśli potrzebujemy pobrać dane z zewnątrz.
...
\newline
\newline
Stan może przyjąć dowolną wartość, \texttt{true}, \texttt{false}, \texttt{null}, może być liczbą, ciągiem znaków (\texttt{string}) lub tablicą.
\begin{lstlisting}[
...
\end{lstlisting}
Jeśli chcemy wykonać dodatkowe czynności po wyrenderowaniu komponentu funkcyjnego, musimy użyć hooka \texttt{useEffect}, który pozwoli nam zaprogramować pożądane ,,Efekty Uboczne''. Możemy o nim myśleć jak o połączeniu metod \texttt{componentDidMount}, \texttt{componentDidUpdate} oraz \texttt{componentWillUnmount} w~jedną. Projekt Gen-mat wykorzystuje ten hook przy pobieraniu danych na temat sprawdzianów, przesyłaniu danych na serwer, oraz ich modyfikacji w~celu stworzenia interfejsu bardziej przystępnego dla użytkownika końcowego.
\begin{lstlisting}[
...
Innym sposobem na przekazanie instrukcji obsługi zdarzenia jest napisanie funkcji obsługi, następnie jej związanie (ang.\textit{ bind}) i wywołanie jej przy pomocy \texttt{this}.
\begin{lstlisting}[
caption={Związanie}
]
...
%####################################################################
\subsection{Formik}
Formik jest biblioteką usprawniającą tworzenie formularzy w Reaccie. Pomaga w~przekazywaniu wartości ze stanów oraz do stanów, walidacji i wyłapywaniu błędów oraz przesyłaniu formularzy.
...
Formik daje nam dostęp do różnych pól oraz metod pomocniczych przydatnych przy tworzeniu formularzy. Początkowe wartości, które ukazują się przy pierwszym załadowaniu formularza możemy przekazaći w~\texttt{initialValues}, podstawowymi metodami pomocniczymi są:
\begin{itemize}
...
Najpierw stworzony zostaje yupowy obiekt o określonych wartościach, a następnie zostaje on przypisany jako \texttt{validationSchema}.
\begin{lstlisting}[
      ...
      axiosInstance
        ...
        .catch((error) => {
          ...
          if (
            ...
            enqueueSnackbar("Nieprawidłowy adres
	 e-mail lub hasło", { 
              variant: 'error',
          })
            helpers.setValues(
...
\end{lstlisting}
Innymi właściowaściami biblioteki Formik używanymi w pojekcie Gen-mat w celu weryfikacji pól formularzy są:
\begin{itemize}
\item \texttt{handleBlur} -- procedura obsługi zdarzenia onBlur - używana, gdy chcemy monitorować, czy konkretne pole zostało skonfigurowane w dowolnym stopniu.
\item \texttt{isSubmitting} -- który sprawdza stan przesyłania formularza, zwraca wartość {\it true}, gdy formularz jest aktualnie przesyłany, w innym wypadku zwraca {\texttt{false}}
\item \texttt{setFieldError} -- pozwalający na ustawienie dodatkowej wiadomośc, gdy w~wybranym polu wystąpi błąd
\end{itemize}
...
\newline
\newline
Jednym z najważniejszych komponentów w React Router jest \texttt{Route}, którego podstawową funkcją jest wyświetlanie właściwej części interfejsu pod podanym adresem URL.
Kolejnym przydatnym komponentem jest \texttt{Switch}, który sprawia, że wyrenderuje się wyłącznie podana ścieżka pod podanym adresem. Zapobiega to niechcianym renderom widoków, które przypadkiem mogą również pasować do podanej ścieżki. Komponent Redirect nadpisuje obecny adres i nawiguje pod nowy adres.
\lstset{
    breaklines=true,                    % sets automatic line breaking
%   breakatwhitespace=false,            % sets if automatic breaks should only happen at whitespace
    prebreak = \raisebox{0ex}[0ex][0ex]{\ensuremath{\hookleftarrow}} % just as an example
...
Dodatkowym atutem jest kompatybilność z szeroką liczbą narzędzi ułatwiających pracę oraz dających więcej możliwości. Fakt, że React jest stale rozwijany oraz używany przez ogromne firmy
