...
Kolejną wygodną opcją jest pominięcie wyboru zadań i zdanie się na automatyczny wybór generatora. Jeżeli nauczycielowi nie spodobają się zadania wybrane przez serwis, może je w każdej chwili zamienić na inne.

Na bieżący moment baza danych składać będzie się wyłącznie z zadań zaczerpniętych z arkuszy Centralnej Komisji Egzaminacyjnej, ponieważ wymyślanie zadań przez założycieli serwisu wiązałoby się z bardzo dużym nakładem pracy oraz ryzykiem niskiej jakości zadań pod względem merytorycznym jak i edukacyjnym. 
...
W rozdziale pt. ,,Implementacja REST API z wykorzystaniem frameworka Django REST'' autorstwa Damian Kuicha opisana zostanie metodyka REST, jak i technologie z niej korzystające. Przedstawione zostaną podstawowe definicje potrzebne do zrozumienia całości pracy. Opiszę paradygmaty jakie musi spełniać aplikacja by móc być nazwana RESTful. W dalszej części przedstawię technologię Django, jak i framework Django REST i opiszę jak ten framework skorelowany jest z ideą REST. 
...
Frontend za to odpowiedzialny jest za część wizualną projektu. Obsługuję on np. wszystkie zapytania wysyłane przez użytkowników, przetwarzanie danych wysyłanych z bazy danych i wyświetlanie ich, komunikację z bazą danych za pomocą API, łączenie się z serwerem za pomocą HTTP, obsługa sesji użytkownika etc . Ta część naszego projektu opiera się na frameworku React, Który bazuje na języku JavaScript. Projekt nasz korzysta, też z frameworka Django REST do tworzenia RESTful API.
