...
Rozdział rozpocznie się od przedstwienia różnych podejść do tworzenia API, ze szczególniem naciskiem na REST API. W rozdziale opisane też zostaną główne zasady tworzenia i utrzymywania REST API z wykorzystaniem frameworka Django REST, od modelu dojrzałości Richardsona, kończąc na filtrowaniu, sortowaniu i stronnicowaniu. Omówiona zostanie także obsługa błędów. 
...
Rozdział będzie zawierał porównanie systemów bazodanowych MySQL, MS SQL Server, Oracle oraz PostgreSQL. Opisane zostaną wady i zalety każdego systemu, ze szczególnym uwzględnieniem używanego przez nas systemu PostgreSQL. Opisany zostanie sposób modelowania danych przechowywanych w bazie danych po stronie frameworka Django. W rozdziale zawarte będą wyjaśnienia relacji między tabelami powstałymi w ramach projektu oraz wpływ tych relacji na wydajność systemu. Omówione zostaną także zagadnienia zachowania integralności i spójności danych. 
...
Rozdział będzie zawierał opis podstawowych komponentów biblioteki Material-UI, ze szczególnym uwzględnieniem formularzy w projekcie Gen-Mat. Zostanie opisane stylowanie komponentów za pomocą makeStyles. W rozdziale znajdzie się też porównanie Material-UI z boostrapem i SASS. 
\item \textbf{Rozdział 4:} "React jako narzędzie do tworzenia interfejsu użytkownika"

Autor: Mateusz Michalski

W tym rozdziale opisana zostanie javascriptowa biblioteka React. Opisane zostaną w szczególności mechanizmy hooków, komponentów i ich właściwości, JSX, stanów, obsługi zdarzeń, renderowania warunkowego. Opisane zostaną mechanizmy komunikacji z API. Opisane zostaną także mechanizmy komunikacji z użytkownikiem. 
